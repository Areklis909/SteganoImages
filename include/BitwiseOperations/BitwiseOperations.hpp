...
public:
  template <typename U> bool checkBit(const U data, const int bitNumber) {
    return (data & ((U)1 << bitNumber)) == ((U)1 << bitNumber);
  }

  template <typename U> bool checkFirstBit(const U data) {
  ...
  uint8_t setBitWithMask(uint8_t byte, uint8_t mask, const bool value,
                         const int bitNumber) {
    byte &= ~mask;
