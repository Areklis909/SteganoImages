...
template<typename ColT, typename ColU>
auto zip(ColT & col1, ColU & col2) {
    if(col1.size() != col2.size()) throw std::runtime_error("zip(): Incorrect size!");
    std::vector<std::pair<typename ColT::value_type &, typename ColU::value_type &> > ret;
    ret.reserve(col1.size());
    std::transform(std::begin(col1), std::end(col1), std::begin(col2), 
        std::back_inserter(ret), [](auto & it1, auto & it2) -> typename decltype(ret)::value_type {
