...
ImageEncryptor::ImageEncryptor(const std::string & imagePath, const std::string & pathToWrite) :
    imageHandler(imagePath, pathToWrite), messageLengthThreshold(10), noThreads(2)
{}

ImageEncryptor::~ImageEncryptor() {}

    std::vector<NsRange::Range> ImageEncryptor::divideIntoRanges(const std::string & message, const int start) {
        ...
        for(size_t i = 0; i < noThreads; ++i) {
            const int currentTaskSize = (i == (noThreads - 1)) ? taskSize + rest : taskSize;
    ...
    std::vector<std::string_view> ImageEncryptor::divideIntoSubmessages(const std::string & message, const std::vector<NsRange::Range> & ranges) {
        
        std::vector<std::string_view> submessages;
        submessages.reserve(ranges.size());
        std::transform(ranges.begin(), ranges.end(), std::back_inserter(submessages), [&](auto & range){
            const int length = range.end() - range.start();
            return std::string_view(message.c_str() + range.start(), length);
        });
        return submessages;
    }

    std::vector<NsTaskDetail::TaskDetail> ImageEncryptor::getTaskDetails(const std::string & message, const int start) {
        ...
        for(const auto complet : zip(submessages, ranges)) {
            ret.push_back(TaskDetail(complet.first, (complet.second * bitsInByte + start)));
        }
        return ret;
    }
...
NsRange::Range ImageEncryptor::getMessageRange(const std::string & message, const int start) {
    using NsRange::Range;
...
void ImageEncryptor::encryptSingle(const std::string & message, const int startPoint) {
...
void ImageEncryptor::encryptData(const std::string & message) {
    ...
    if(message.length() < messageLengthThreshold) {
        ...
        for(size_t i = 0; i < noThreads; ++i) {
            tasks.push_back(ParallelTask(&ImageEncryptor::encryptMulti, this, details.at(i)));
        }
