...
ProcessingMode ProcessingMode::fromString(const std::string & strMode) {
    if(encodingStr.compare(strMode) == 0) {
        ...
        throw std::runtime_error("Failed to translate from std::string to ProcessingMode!");
    }
}
...
std::string ProcessingMode::toString() {
    ...
    switch(mode) {
        ...
        default:
        {
            throw std::runtime_error("Wrong processing mode type detected during translation!");
        }
    }
    return ret;
}
